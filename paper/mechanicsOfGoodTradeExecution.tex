...
%%	\end{figure}
%	
%	When facing the unavailability of future price movements, one has to introduce a model for the evolution of $\fundamentalPrice$ in order to gain some predicting power. Moreover, the classical optimisation of control theory is of the expected quantity $\Expectation J(\inventory)$ rather than of $J(\inventory)$, and this is also model dependent. Two alternatives are offered: choosing a \emph{static} strategy would mean to decide the entire inventory trajectory at the initial time;\footnote{Mathematically, this is formulated by considering $\inventory$ as a deterministic path.} choosing a \emph{dynamic} strategy would instead mean to allow for adjustments of $\inventory(t)$ depending on the information available at $[0,t]$, the time interval spanning from the beginning of the trading period up to the present.\footnote{Mathematically, this is formulated by considering $\inventory$ as a stochastic process adapted to the filtration of the price $\fundamentalPrice$. }
%	
%	In the static case, the model dependency is moderate, in the sense that only the expected price path $t\mapsto \Expectation \fundamentalPrice_t$ enters the computations. Hence, there exist different models for $\fundamentalPrice$ that produce the same optimal static inventory trajectory. Notably, this is the case for price evolutions with different volatilities. In Figure \ref{fig.aPosterioriTwoVolatilities} the two price paths were simulated as arithmetic Brownian motions with the same drift -- actually null -- but different volatilities. The static strategy in the dashed line does not distinguish  the two, whereas we observe rather different shapes of the a posteriori optimal inventories. 
%	
...
%	Let $\lbrace \fundamentalPrice_t:\, 0\leq t\leq T\rbrace$ be the time evolution of the asset fundamental price, defined on some probability space $\probabilitySpace$. It is assumed to be a semimartingale, possibly with jumps. We assume that for all $0\leq t\leq T$ the second moment of $\fundamentalPrice_t$ is finite and that the maps $t\mapsto \Expectation\fundamentalPrice_t$ and $t\mapsto \Expectation\fundamentalPrice\squared_t$ are in $L^1[0,T]$. Let $\lbrace \sigmaAlgebra_t: \, 0\leq t\leq T\rbrace$ be the minimal $\Prob$-completed right-continuous filtration generated by $\fundamentalPrice$. 
%	
...
%	
	...
	It thus becomes apparent the paramount role of the expected trajectory $t\mapsto \Expectation[\fundamentalPrice_t]$; this reinforced our focus on the concept of robustness introduced in Definition \ref{defi.marginalRobustness}. 
